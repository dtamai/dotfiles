...
tags = {
   names =  { "[₆]"     , "[₇]"     , "[₈]"     , "[₉]"     , "[₀]"},
   layout = { layouts[2], layouts[3], layouts[4], layouts[3], layouts[2] }
}
...
globalkeys = awful.util.table.join(
    -- Lock screen
    awful.key({ altkey, "Ctrl" }, "l", function() os.execute("mate-screensaver-command --lock") end),
    -- Take a screenshot
    ...
    awful.key({ }, "XF86AudioRaiseVolume",
        function ()
          os.execute(string.format("amixer -D %s set %s 5%%+", "pulse", volumewidget.channel))
          volumewidget.update()
        end),
    ...
    awful.key({ }, "XF86AudioMute",
        function ()
          os.execute(string.format("amixer -D %s set %s toggle", "pulse", volumewidget.channel))
          volumewidget.update()
        end),
